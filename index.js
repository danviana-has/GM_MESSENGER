/**
 * GM MESSENGER - Aplicação Full-Stack em Cloudflare Worker Único
 * Contém Backend, Integração com E-mail Corporativo, Validação de Código no KV e a Interface Web Completa.
 */
...
export default {
  async fetch(request, env) {
    ...
    // RESTRIÇÃO DE SEGURANÇA E HEADER CORS
    const corsHeaders = {
    ...
    /* ===================================================================
       1. ROTA BACKEND: SOLICITAR CÓDIGO ALEATÓRIO E ENVIAR POR E-MAIL
       =================================================================== */
    if (url.pathname === "/api/auth/send-code" && request.method === "POST") {
      try {
        const { email } = await request.json();

        // Validação estrita de provedor
        if (!email || !email.endsWith("@email.gmcorporation.com.br")) {
          return Response.json(
            { success: false, error: "Acesso bloqueado: É obrigatório utilizar seu e-mail do provedor corporativo (@email.gmcorporation.com.br)." },
        ...
        // Armazena no Cloudflare KV com expiração de 10 minutos (600s)
        await env.OTP_STORE.put(`otp:${email}`, generatedCode, { expirationTtl: 600 });

        // Disparo real via API do Provedor Interno
        ...
        if (mailResponse.ok && mailData.success) {
          return Response.json({ success: true, message: "Código enviado com sucesso para a sua caixa de entrada!" }, { headers: corsHeaders });
        } else {
          return Response.json(
    ...
    /* ===================================================================
       2. ROTA BACKEND: VALIDAR O CÓDIGO DIGITADO PELO USUÁRIO
       =================================================================== */
    if (url.pathname === "/api/auth/verify-code" && request.method === "POST") {
      try {
        ...
        if (storedCode === code.trim()) {
          // Apaga o código do KV após a verificação aprovada
    ...
    /* ===================================================================
       3. ROTA FRONTEND: INTERFACE HTML/CSS/JS SERVIDA DENTRO DO WORKER
       =================================================================== */
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif; }
    ...
    .message-time { font-size: 0.65rem; color: var(--text-dim); text-align: right; margin-top: 0.2rem; display: block; }
...
<body>
  ...
  <div id="auth-screen">
    <div class="auth-card">
      ...
      <!-- PASSO 2: COPIAR E COLAR O CÓDIGO (OCULTO INICIALMENTE) -->
  ...
  <script>
    ...
    // PASSO 2: Aprovação do código copiado e colado pelo usuário
